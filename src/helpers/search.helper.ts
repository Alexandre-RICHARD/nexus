  ...
  calculateDistance: (a: string, b: string): number => {
    ...
    for (; x < lb - 3; ) {
      ...
      for (let y = 0; y < len; y += 2) {
        dy = vector[y];
        ay = vector[y + 1];
        d0 = SearchHelper.getMin(dy, d0, d1, bx0, ay);
        d1 = SearchHelper.getMin(d0, d1, d2, bx1, ay);
        d2 = SearchHelper.getMin(d1, d2, d3, bx2, ay);
        dd = SearchHelper.getMin(d2, d3, dd, bx3, ay);
        vector[y] = dd;
        d3 = d2;
        d2 = d1;
        d1 = d0;
    ...
    for (; x < lb; ) {
      ...
      for (let y = 0; y < len; y += 2) {
        dy = vector[y];
        dd = SearchHelper.getMin(dy, d0, dd, bx0, vector[y + 1]);
        vector[y] = dd;
        d0 = dy;
      }
    }
